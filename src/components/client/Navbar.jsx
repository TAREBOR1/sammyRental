...
const Navbar = () => {
    ...
    return (
        <>
            ...
            {/* Mobile Menu */}
            <div className={`fixed h-screen inset-0 z-[9999] bg-white overflow-hidden p-4 flex flex-col items-center justify-center text-gray-800 transform transition-transform duration-400 ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <button
                    className="absolute top-5 left-5"
                    onClick={() => setIsMenuOpen(false)}
                >
                ...
                {navLinks.map((link, i) => (
                    <Link
                        ...
                        to={link.path}
                        onClick={() => setIsMenuOpen(false)}
                        className="text-lg font-semibold hover:text-black mt-4"
                    >
                        {link.name}
                    </Link>
                ))}
